...
$items_query = $conn->query("SELECT * FROM services");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $delete_stmt = $conn->prepare("DELETE FROM services");
    $delete_stmt->execute();
    ...
    foreach ($_FILES['items']['tmp_name'] as $index => $tmp_name) {
        ...
        if (isset($tmp_name['image']) && is_uploaded_file($tmp_name['image'])) {
            ...
            if (move_uploaded_file($tmp_name['image'], $target_file)) {
                ...
                $insert_stmt = $conn->prepare("INSERT INTO services (image_link, title, description) VALUES (?, ?, ?)");
...
<h1 class="bg-white flex justify-center text-2xl font-bold px-2 py-4">Add Our Services</h1>
