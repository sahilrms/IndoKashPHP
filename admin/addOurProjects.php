...
$project_images = $conn->query("select * from project_media");
$existing_project_images = $project_images->fetch_all(MYSQLI_ASSOC);
            ...
            <?php foreach ($existing_items as $index => $item): ?>
                ...
                <div class="border rounded-lg shadow-2xl bg-white w-5/6 mx-auto flex flex-col space-y-1 my-2 p-4">
                    ...
                    <div class="border rounded-lg shadow-2xl bg-white w-5/6 mx-auto flex flex-col space-y-1 my-2 p-4">
                        ...
                        <div class="w-full py-2 flex flex-col">
                            <label for="media_url_<?php echo $index ?>" class="text-gray-600">Media files:</label>
                            <input id="media_url_<?php echo $index ?>" name="items[<?php echo $index ?>][media_url]"
                                type="file" multiple
                                class="px-3 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300">
                        </div>
                        ...
                        foreach ($existing_items as $index => $item) {
                            // Filter existing_project_images based on project_id
                            $filtered_images = array_filter($existing_project_images, function ($image) use ($item) {
                                return $image['project_id'] == $item['id'];
                            });

                            echo '<div class="flex flex-wrap gap-2">';
                            // Check if there are any filtered images
                            if (!empty($filtered_images)) {
                                // Display all images for the project
                                foreach ($filtered_images as $image) {
                                    echo '<img src="' . $image['media_url'] . '" width=150 />';
                                }
                            }
                            echo '</div>';
                        }
        ...
        <div class="border rounded-lg shadow-2xl bg-white w-5/6 mx-auto flex flex-col space-y-1 my-2 p-4">
            ...
            <div class="flex flex-wrap">
                            <div class="w-2/5 py-2 flex flex-col">
                                ...
                                <input id="owner_${itemCount}"  type="text" name="items[${itemCount}][owner_name]" required
                                    class="px-3 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300">
                            </div>
                            <div class="w-2/5 py-2 mx-auto flex flex-col">
